...
func GenerateCouponCode(r *http.Request) string {
	...
	rows, err := DB.Query("INSERT INTO `coupon` (code, expiry_date, availability_count, product_id, promo_type, discount_fixed, discount_variable) VALUES (?, ?, ?, ?, ?, ?, ?)", coupon_code, data["expiry_date"], data["availability_count"], data["product_id"], data["promo_type"], data["discount_fixed"], data["discount_variable"])
	checkError(err)
	log.Println(rows)
	return coupon_code
}
...
func GetCouponDetails(r *http.Request) map[string]string {
	...
	rows, err := DB.Query("SELECT * FROM `coupon` WHERE code=?", code)
	checkError(err)
	cols, _ := rows.Columns()
...
func ConsumeCoupon(r *http.Request) string {
	...
	rows1, err := DB.Query("UPDATE `coupon` SET availability_count=? WHERE id=?", availability_count-1, coupon_id)
	checkError(err)
	log.Println(rows1)
	rows2, err := DB.Query("INSERT INTO `order` (coupon_id, client_id, status) VALUES (?, ?, ?)", coupon_id, rand.Intn(100), "Success")
	checkError(err)
	log.Println(rows2)
